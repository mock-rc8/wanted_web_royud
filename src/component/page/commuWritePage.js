...
export default function CommuWritePage(){
    ...
    useEffect(()=> {
        if(selected_all_tags.length < 1 || !title_value || !desc_value){
            set_commu_write_top_btn("commu_write_top_btn")
        }
        if(selected_all_tags > 0 && title_value && desc_value){
            ...
            <div className="MyInfo_Main">
                <div className="commu_write_top">
                    <div className="commu_write_top_wrap">
                        <button className={commu_write_top_btn}>
                            등록하기
                        </button>
                    </div>
                </div>
                                ...
                                ?<ul className="selected_all_tags_lists">
                                    {
                                        tag_of_touch.map((tags) => (
                                            selected_all_tags.includes(tags.tagIdx.toString())
                                            ?<li
                                            key={tags.tagIdx}
